...
import {
  ArenaCategoryModalPropTypes,
  ArenaMessageCompPropTypes,
  ContestModalCompPropTypes,
  HeighestStakeListArenaComponentPropTypes,
  gameComponentPropType,
} from "../types";
...
export const HeighestStakeListArenaComponent = ({
  ...
  return (
    <View className="space-y-1 w-[118px] ">
      <Image source={logoImage} className="w-[118px] h-[153px] rounded-md" />
      <View className="w-[118px] ">
        <Text className="text-gray-950 text-[10px] font-bold font-['GeneralSans-Bold'] leading-[18px]">
          {heading}
        </Text>
        <View className="flex-row justify-between items-center w-full">
          <View className="flex-row space-x-1">
            <View className="w-[24px] h-[24px]">
              <Image source={userImage} className="w-[24px] h-[24px]" />
            ...
            <View className="h-[24px] justify-between">
              <Text className="text-gray-950 text-[8px] font-medium font-['General Sans Variable'] leading-3">
                @{userName}
              </Text>
              <View className="flex-row">
                <Text className="text-gray-950 text-[8px] font-medium font-['General Sans Variable'] leading-[8px]">
                  Stake:$
                </Text>
                <Text className="text-gray-950 text-[8px] font-medium font-['General Sans Variable'] leading-[8px]">
                  {amount}
...
export const ArenaContestantComponent = ({
  active,
  amount,
  img,
  name,
    ...
    <View className="w-full flex-row justify-between items-start mt-[18px] ">
      <View className="flex-row space-x-2 items-start">
        ...
        <View className="">
          <AppTextHeading
            text={name}
            classText="text-gray-950 text-[16px] font-medium font-['General Sans Variable'] leading-[16px]"
          />
          <View className="flex-row items-center space-x-1">
            <Text className="text-neutral-400 text-[12px] font-medium font-['GeneralSans-Regular'] leading-[21px]">
              {userName}
            </Text>
            <Text className="text-neutral-900 text-[12px] font-medium font-['GeneralSans-Regular'] leading-[12px]">
              vs
            </Text>
        ...
        <View
          className={`px-1 py-0.5 bg-indigo-50 rounded-[39px] border  justify-center border-sky-500 items-center  ml-4 ${active && "border-green-400"}`}
        >
          <Text
            className={` text-[6px] font-medium font-['GeneralSans-Regular '] text-sky-500 leading-3 ${active && "text-lime-700"}`}
          >
      ...
      <View>
        <Text className="text-neutral-400 text-[12px] font-medium font-['GeneralSans-Regular'] leading-[12px]">
          {time} ago
        </Text>
        <Text className="text-sky-500 text-[15px] font-medium font-['GeneralSans-Medium Variable'] leading-[15px] py-1">
          ${amount}
        </Text>
      </View>
    </View>
...
export const ArenaCategoryModal = ({
  showModal,
  setShowModal,
  ArenaCategoryModalDataList,
  choosedCategory,
  setChoosedCategory,
}: ArenaCategoryModalPropTypes) => {
  ...
  const [filteredByHashTagData, setFilteredByHashTagData] = useState(
    filterByHashTagArenaArray
  );
  ...
  useEffect(() => {
    const selectedHashTag = filterByHashTagArenaArray.filter(
    ...
    <Modal animationType="slide" transparent={true} visible={showModal}>
      {showFilterModal ? (
        ...
        >
          <View className="px-[16px] pt-[20px] pb-[10px]  w-full bg-white  rounded-t-[30px]  z-10">
            <View className="items-center justify-center w-full  bg-opacity-100 opacity-100 mt-[10px] mb-8">
              ...
              <AppTextHeading
                text="Filter"
                classText="text-center text--[18px]"
              />
            </View>
            <View className="mb-8">
              <AppTextHeading
              ...
              />
              <AppTextContent
                text="All result from your selections ranging from main to sub categories"
                classText="text-[11px] w-full leading-[13px]"
              />
            </View>
            ...
            <View className="flex-row justify-between items-center">
              ...
              >
                <Text className="text-violet-700 text-[10px] font-medium font-['GeneralSans-Regular'] leading-[18px]">
                  {categoryData}
                </Text>
              </View>
              ...
              >
                <Text className="text-lime-700 text-[10px] font-medium font-['GeneralSans-Regular'] leading-[18px]">
                  {subCategoryHeading1Data}
                </Text>
              </View>
              <View className="w-[33px] h-[0px] origin-top-left rotate-[123.66deg] border border-neutral-400"></View>
              ...
              >
                <Text className="text-yellow-500  text-[10px] font-medium font-['GeneralSans-Regular'] leading-[18px]">
                  {subCategoryHeading2Data}
                </Text>
              </View>
            </View>
            {/* filter list ends */}
            ...
            <View className="mt-8 mb-8">
              <View className="">
                <AppTextHeading
                  text="Filter By Hashtags"
                  classText="text-[18px]"
                />
                <AppTextContent
                  text="Choose atleast 2 Hashtags to help your search better"
                  classText="text-[11px] w-full leading-[13px]"
                />
              </View>

              <View className="flex-row justify-between mt-4">
                {filteredByHashTagData.slice(0, 4).map((item) => (
                  <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => {
                    ...
                    key={item.id}
                    className={`w-[78px] h-[38px] rounded  justify-center items-center ${item.active && "bg-indigo-50"}`}
                  >
                    <Text
                      className={`text-neutral-400 text-sm font-semibold font-['GeneralSans-Regular'] leading-[18px] ${item.active && "text-sky-500"}`}
                    >
                      {" "}
                      #{item.text}
                    </Text>
              ...
              <View className="flex-row justify-between mt-2">
                {filteredByHashTagData.slice(4, 8).map((item) => (
                  <TouchableOpacity
                    onPress={() => {
                      selectFilteredDataFunct(item.id);
                    }}
        ...
        <TouchableOpacity
          ...
          className="h-screen  justify-end border border-yellow-500"
        >
          <View className="px-[16px] pt-[20px] pb-[10px]  w-full bg-white  rounded-t-[30px]  z-10">
            ...
            <ScrollView
              ...
              className="space-y-2 "
            >
              {activeSubCateryList.length > 0 ? (
                <View>
                 <View className="items-center justify-center w-full mt-[10px] mb-[20px]">
              <TouchableOpacity
                onPress={() => {
                  // activeSubCateryList.length > 0
              ...
              <AppTextHeading
                    text={subCategoryHeading1Data}
                    classText="text-center text-[18px]"
                  />
            </View>
                 <View className="space-y-2">
                 {activeSubCateryList.map((item) => (
                    <TouchableOpacity
                      onPress={() => {
                      ...
                      activeOpacity={0.8}
                      key={item.key}
                      className="flex-row h-[41.5px]  items-center space-x-2 "
                    >
                      <Image
                        source={item.img}
                        className="w-[61px] h-[41.5px] rounded-md"
                      />
                      <View className="flex-1 h-full justify-start space-y-2">
                        <AppTextHeading
                          text={item.heading}
                          classText="text-sm font-[GeneralSans-Regular] font-black"
                        />
                      </View>
                    </TouchableOpacity>
                  ))}
            ...
            >
              {ArenaCategoryModalDataList?.map((item) => (
                <TouchableOpacity
                  ...
                  key={item.key}
                  className="flex-row h-[70px]  items-center space-x-2"
                >
                  <Image source={item.img} className="w-[108px] h-[70px]
                  rounded-md" />
                  <View className="flex-1 h-full justify-start space-y-2">
                    <AppTextHeading
                      text={item.heading}
                      classText="text-sm text-[18px] font-[GeneralSans-Regular] font-md "
                    />
                    <AppTextContent
                      text={item.content}
                      classText="text-[10px] w-full leading-[12px] text-left  pt-1"
                    />
                  </View>
                  <TouchableOpacity
                    className="h-full w-[31px] bg-purple-700 justify-center items-center
...
export const ArenaMessageComp = ({
  ...
  return (
    <View className=" w-full mt-4">
      <AppTextContent
        text={time}
        classText="text-center  text-[7px] text-neutral-400"
      />
...
export const ContestModalComp = ({
  ...
  return (
    <Modal animationType="slide" transparent={true} visible={showModal}>
      <View
        className="h-screen items-center justify-center px-[16px]"
        style={{
          backgroundColor: "rgba(78,78,78,0.2)",
        }}
      >
        <View className="p-2 rounded-xl  bg-white  pt-12 pb-4">
          <TouchableOpacity
            activeOpacity={0.8}
            onPress={() => {
              console.log("pressed");
          ...
          <View className="flex-col justify-center items-center ">
            ...
            <AppTextContent
              text="You don’t have any money in your wallet to join this contest"
              classText="text-center mt-4"
            />
          </View>
          ...
          <View className="w-full  flex-row">
            <AppButton
              text="Deposit Now"
              ButtonViewStyle="bg-sky-500 w-full mt-4"
            />
          </View>
        </View>
      </View>

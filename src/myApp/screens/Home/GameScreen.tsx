...
import {
  filterModalDataByHash,
  gameData,
  gameMessageCompData,
  gameSearchModelResentData,
  gameSliderData,
} from "utils/data";
  ...
  return (
    <View className="flex-1 w-full">
              ...
              >
                ...
                <View className="mt-4">
                  ...
                  <View className="flex-row items-center justify-between">
                    {gameSearchModelResentData.map((item) => (
                      <View
                        key={item.key}
                        className="flex-row items-center space-x-1 mt-4"
                      >
                        <Image
                          source={item.img}
                          className="w-[50px] h-[30px]"
                        />
                        <Text className="text-gray-950 text-sm font-medium font-['GeneralSans-Regular'] leading-[14px]">
                  ...
                  <View className="flex-row justify-between items-center mt-4">
                    {filterModalDataByHash[0].map((i, k) => (
                      ...
                      >
                        <Text
                          className="text-sky-500 text-[10px] font-medium font-['General Sans Variable'] leading-[18px]"
                  ...
                  <View className="flex-row justify-between items-center mt-4">
                    {filterModalDataByHash[0].map((i, k) => (
                      <View
                        ...
                        className="py-1 bg-indigo-50 rounded-2xl    justify-center items-center"
                      >
                        <Text className="text-sky-500 text-[10px] font-medium font-['General Sans Variable'] leading-[18px]">
                          {i.hash}
                        </Text>
                      </View>
                    ))}
                  ...
                  <AppButton
                    ...
                    text="Apply"
                    ButtonViewStyle="bg-sky-500 mt-[32px]"
                  />
                </View>
              </View>
            </View>
      ...
      <View className="w-full flex-row items-center justify-between px-[16px]  mt-[170px]">
        <View>
          <AppTextHeading
            classText="text-gray-950 text-[18px] font-medium  leading-normal"
            text={"Available Contest"}
          />
        </View>
      ...
      <ScrollView className="flex-1 w-full  px-[16px]">
        {appData.length > 0 ? (
          appData.map((item: gameComponentPropType) => (
            <GameComponent {...item} />
          ))
        ) : (

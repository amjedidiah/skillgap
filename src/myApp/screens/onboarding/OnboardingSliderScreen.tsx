...
const handleSnapToItem = (index) => 
{
   ...
   return   <View
    ...
    >
      ...
      <View className={`bg-transparent  absolute  bottom-0 h-screen w-screen  justify-end `} 
 
      > 
      <View className="rounded-tl-2xl px-2 rounded-tr-2xl bg-white "
      style={{
      ...
      <View className="flex-row items-center bg-white  justify-between w-full px-8 absolute bottom-4 pb-6">
        <View className="flex-row space-x-2">
        {
            item.progressArray.map(k => {
                if(k.i === item.key){
                return     <View key={k.i} className="w-[30px] h-1 bg-gray-950 rounded-lg" />
                }else{
                 return     <View key={k.i} className="w-4 h-1 bg-gray-300 rounded-lg" />
                }
            })
        }
        </View>

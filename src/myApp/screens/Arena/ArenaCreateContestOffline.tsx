import { View, Text, Image, TouchableOpacity, TextInput, ScrollView, StatusBar, Modal, Alert } from 'react-native'
...
const ArenaCreateContestOffline= () => {
  ...
  const appUserStore = useSelector(data => data?.authReducer?.user)
    ...
    const [opponentSkillGapArray, setOpponentSkillGapArray] = useState<string[]>([])
    const [opponentSkillGapTag, setOpponentSkillGapTag] = useState<string>("")
  ...
  console.log("opponentSkillGapArray ", opponentSkillGapArray)
    ...
    {/* header section ends */}
      ...
      <View className='items-start  justify-between flex-row w-full '>
        <View className='flex-1 justify-start'>
          <AppTextHeading text='Group Contest' classText='text-[16px] mb-1 leading-[16px]' />
...
{/* firtst switch ends*/}
      ...
      <View className={`w-full mt-4 ${translate && "bg-slate-200 opacity-20"}`}>
            ...
            <View className="items-start space-y-[10px] w-full">
             ...
             {
                opponentSkillGapArray.length > 0 &&  <View className='w-full mb-4 space-y-2'>
                {
                    opponentSkillGapArray.map(item  => <View key={item?.id} className="w-full h-[42px] px-4 text-white  flex-row items-center justify-between space-x-2 bg-white rounded-[30px] shadow border border-gray-300">
                        <View>
                        <AppTextContent text={item?.text} classText='text-[12px] leading-[18px] w-full' />
                        </View>
                        <TouchableOpacity
                        className="p-2"
                        activeOpacity={0.5}
                        onPress={() => {
                           ...
                           const newData = opponentSkillGapArray.filter(i  => i.id !== item.id )
                           setOpponentSkillGapArray([...newData]);
                        }}>
                        <Image
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value } }) => (
                  ...
                  ">
                    <TextInput
                      onChangeText={(data)=> {
                        setOpponentSkillGapTag(data.trim())
                       setForm({
            ...
            <TouchableOpacity
            disabled={!opponentSkillGapTag}
            onPress={() => {
                // check if skill gap tag already added
               const skillGapTagFound = opponentSkillGapArray.filter(item => item?.text.toLowerCase() === opponentSkillGapTag.toLowerCase())
                if(skillGapTagFound.length > 0) {
                      Alert.alert("Error",`Skill gap tag ${opponentSkillGapTag} already added`)
                      return
                }

             setOpponentSkillGapArray([...opponentSkillGapArray, {
                id: opponentSkillGapArray.length,
                text: opponentSkillGapTag
             }])
            }}
            activeOpacity={0.5}
            className='flex-row items-center justify-center space-x-2 border  px-2.5  border-[#1D9BF0] rounded-[40px] h-[42px]'>
            <AntDesign name="plus" size={15} color="#1D9BF0" />
            <Text className='font-[GeneralSans-Regular] text-sky-400 text-[16px]'>Add Opponent</Text>
            </TouchableOpacity>
          </View>
          {/*skillgap tag  end */}
...
<View>
{
 ...
 <View className='rounded-[20px] bg-white py-5 mt-4  px-4'>
  ...
  <View className='rounded-[20px] bg-white py-10 mt-4  px-4 '>
         {/* first switch starts */}
      <View className='items-start  justify-between flex-row w-full mb-2'>
        <View className='flex-1 justify-start'>
          <AppTextHeading text='Opponent’s skillgap tag' classText='text-[16px] mb-1 leading-[16px]' />
          <AppTextContent text='Type your opponent tag here' classText='text-[12px] leading-[18px] w-full' />
        </View>
...
{/* firtst switch ends*/}
  ...
  <View className='rounded-[20px] bg-white py-10 mt-4  px-4 '>
        ...
        <View className='w-full'>
          <AppTextHeading text='Referee' classText='text-[16px] mb-1 leading-[16px]' />
          <AppTextContent text='Use email address to sent invite if it’s a first time user. Use skillgap tag if it’s an existing user.' classText='text-[12px] leading-[18px] w-full' />
        </View>
      <View>
      <AppTextHeading text='Skill gap tag' classText='text-[16px] mt-4 mb-2 leading-[16px]' />
...
{errors.skillGapTag && (
 ...
 >
  ...
  <View className='flex-row items-center justify-center space-x-2 my-2'>
    <View className='h-[2px] flex-1 bg-slate-100'></View>
    <Text>Or</Text>
    <View className='h-[2px] flex-1 bg-slate-100'></View>
  </View>
      <View>
      <AppTextHeading text='Email Address' classText='text-[16px] mb-2 leading-[16px]' />
...
export default ArenaCreateContestOffline